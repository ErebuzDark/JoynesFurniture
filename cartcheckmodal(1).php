...
$sqlus = "SELECT * FROM addcart WHERE userID = $userID";
$resus = mysqli_query($conn, $sqlus);



$userID = $_SESSION['userID'];
$sqlid = "SELECT SUM(cost) FROM addcart WHERE userID = ?";
$stmt = $conn->prepare($sqlid);
$stmt->bind_param('i', $userID);
$stmt->execute();
$resultid = $stmt->get_result();
$row5 = $resultid->fetch_assoc();
$number = $row5['SUM(cost)'];
$formattedNumber = number_format($number, 0, '.', ',');
$costs = $formattedNumber;

// $sqltot = "SELECT SUM(cost) AS total_cost FROM furnituretbl WHERE fID = $userid";
// $stmt = $conn->prepare($sqltot);
// $resultot = $stmt->get_result();
// $rows = $resultot->fetch_assoc();
// $tot = $rows['SUM(cost)'];
          ...
          <div class="card card-stepper" style="border-radius: 16px;">
            ...
            while ($row = mysqli_fetch_assoc($resus)) {
              ...
              <div class="card-body p-3">
                <div class="d-flex flex-row md1 mb-3 pb-2 mt-4">
                  <div class="col-md-10 col-lg-10 col-xl-10 flex-fill ms-4 ">
                    <h5 class="bold"><?php echo $row['prodName']; ?></h5>
                    <p class="text-muted"> Qt: <?php echo $row['quantity']; ?> item </p>
                    <h4 class="mb-3">&#8369;<?php echo $cost; ?></h4>
                  </div>
                  <div class="col-md-2 col-lg-2 col-xl-2 flex-fill me-5">
                    <img class="align-self-center img-fluid " src="./up/<?php echo $row['image']; ?>" width="100">
                  </div>
                  <br><br>
                </div>
            ...
            <input type="hidden" name="cost" value="<?php echo $costs; ?>">
            <input type="hidden" name="date" value="<?php echo $rows['date']; ?>">
            <input type="hidden" name="status" value="On Queue">

            <div class="card-footer p-4">
              <div class="d-flex justify-content-between">
                <h3 class="d-flex textp">Total: &#8369;<?php echo $costs; ?></h3>
              </div>
              <br>
              <div class="d-flex justify-content-between">
