...
if (isset($_GET['id'])) {
    ...
    $furniture = "SELECT * FROM furnituretbl WHERE fID = ?";
    if ($stmt = mysqli_prepare($conn, $furniture)) {
        ...
        if ($row = mysqli_fetch_assoc($result)) {
            ...
            $sql = "INSERT INTO archived (ID, name, image, quantity, description, cost, date) 
                    VALUES (?, ?, ?, ?, ?, ?, ?)";
            if ($stmt = mysqli_prepare($conn, $sql)) {
                // Bind parameters and execute insert query
                mysqli_stmt_bind_param($stmt, 'issssss', $id, $furName, $filename, $quantity, $des, $cost, $date);
                if (mysqli_stmt_execute($stmt)) {
                    // Prepare DELETE query to remove from furnituretbl
                    $deleteSql = "DELETE FROM furnituretbl WHERE fID = ?";
